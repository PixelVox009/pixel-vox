...
const MIN = 800;
const MAX = 1000;
function splitStoryIntoSegments(story: string, numSegments: number) {
  // Kiểm tra đầu vào
 ...
 * @returns {string[]} Mảng chứa các đoạn đã được chia
 */
function splitStoryByWords(story: string, numSegments: number) {
  // Kiểm tra đầu vào
  if (!story || typeof story !== "string") {
    throw new Error("Vui lòng cung cấp nội dung câu truyện");
  }
...
function autoSplitStory(story: string, customNumSegments = null) {
  ...
  console.log(
    `Đã chia thành ${segments.length} đoạn (mỗi đoạn từ MIN-MAX ký tự)`
  );
 ...
 * @returns {string[]} - Mảng chứa các đoạn đã được chia
 */
function splitStoryByCharLength(story: string) {
  // Kiểm tra đầu vào
  if (!story || typeof story !== "string") {
    throw new Error("Vui lòng cung cấp nội dung câu truyện");
  }
    ...
    // Nếu đoạn hiện tại chưa đạt MIN ký tự nhưng thêm câu mới sẽ vượt quá MAX ký tự
      ...
      for (let j = 0; j < words.length; j++) {
        if (
          ...
          currentSegment += partialSentence
            ? " " + partialSentence
            : partialSentence;
          segments.push(currentSegment);
  ...
  // Log thông tin về các đoạn
  finalSegments.forEach((segment, index) => {
    console.log(
      `Đoạn ${index + 1} (${segment.length} ký tự, ${
        segment.split(/\s+/).filter((w) => w.trim() !== "").length
      } từ):`
    );
    // console.log(segment);
  });
...
//
//   console.log(`Độ dài chuỗi: ${storyText.length} ký tự`);
//   console.log(
//     `Tổng số từ: ${
//       storyText.split(/\s+/).filter((w) => w.trim() !== "").length
//     } từ`
//   );
//   console.log(
//     `Đã chia thành ${segments.length} đoạn (mỗi đoạn từ MIN-MAX ký tự)`
